...
//Funciton used to simulate random
...
void main() {
    ...
    // float strength = square1 * suqare2; 
    ...
    // vec2 gridUv = vec2(
    //     floor(vUv.x * 10.0) / 10.0 ,
    //     floor((vUv.y + 0.5 * vUv.x)* 10.0) / 10.0
    // );
    // float strength = random(gridUv);
    ...
    vec3 uvColor = vec3(vUv, 1.0 + sin(uTime));
    vec3 mixedColor = mix(blackColor, uvColor, strength);

    gl_FragColor = vec4(mixedColor, 1.0);

    // Black adn white version
    // gl_FragColor = vec4(vec3(strength), 1.0);
